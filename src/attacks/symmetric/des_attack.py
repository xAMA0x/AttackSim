...
def brute_force_des(ciphertext: bytes, plaintext: bytes, key_bits: int, 
    ...
    for key_int in range(max_keys_to_test):
        # Convertit l'entier en octets (clé)
...
def run_validation_mode():
    ...
    # Demande la clé en hexadécimal
    key_hex = Prompt.ask("[yellow]Entrez la clé DES en hexadécimal (16 caractères = 8 octets)[/yellow]")
    
    try:
        if len(key_hex) != 16:
            console.print("[bold red]✗ La clé doit faire exactement 16 caractères hex (8 octets)[/bold red]\n")
            return
        
        true_key = bytes.fromhex(key_hex)
        
        # Calcule le nombre de bits effectifs de la clé
        # Convertit la clé en entier pour trouver le nombre de bits nécessaires
        key_int = int.from_bytes(true_key, 'big')
        effective_bits = key_int.bit_length() if key_int > 0 else 8
        
        console.print(f"[cyan]Clé acceptée: {key_hex}[/cyan]")
        console.print(f"[cyan]Nombre de bits effectifs: {effective_bits}[/cyan]\n")
    except ValueError:
        console.print("[bold red]✗ Format hexadécimal invalide ![/bold red]\n")
        return
    ...
    # Limite à 22 bits max pour éviter des temps trop longs, mais teste au moins les bits effectifs
    # Si la clé dépasse 22 bits, on teste quand même jusqu'à 22 bits pour la démo
